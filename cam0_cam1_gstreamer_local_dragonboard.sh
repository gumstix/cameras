...
gst-launch-1.0 v4l2src device=/dev/video0 ! 'video/x-raw,format=UYVY,width=640,height=480,framerate=10/1' ! glimagesink
fi
if [ $key -eq 2 ]; then
...
gst-launch-1.0 v4l2src device=/dev/video1 ! 'video/x-raw,format=UYVY,width=640,height=480,framerate=10/1' ! glimagesink
...
# Set the pipeline format for cam1
media-ctl -d /dev/media1 -V '"ov5640 1-0074":0[fmt:UYVY2X8/640x480 field:none],"msm_csiphy1":0[fmt:UYVY2X8/640x480 field:none],"msm_csid1":0[fmt:UYVY2X8/640x480 field:none],"msm_ispif1":0[fmt:UYVY2X8/640x480 field:none],"msm_vfe0_rdi1":0[fmt:UYVY2X8/640x480 field:none]'
gst-launch-1.0 v4l2src device=/dev/video0 ! 'video/x-raw,format=UYVY,width=640,height=480,framerate=10/1' ! textoverlay text='CAM0' halignment=left valignment=top font-desc='Sans Italic 24' ! videomixer name=mix sink_1::xpos=0 sink_1::ypos=480 sink_1::zorder=3  ! glimagesink v4l2src device=/dev/video1 ! 'video/x-raw,format=UYVY,width=640,height=480,framerate=10/1' ! textoverlay text='CAM1' halignment=left valignment=top font-desc='Sans Italic 24' ! mix.
